...
public abstract class ListDiff<E> implements IDiff {
	 ...
	 * type-unsafe way. Derived classes can override this method to return the
	 * identical results in a type-safe way. Ultimately it should be required
	 * that this method be implemented in derived classes. At that time the
	 * <code>getDifferences</code> method can be deprecated.
	 * 
	 * @return a ListDiffEntry array representing the differences in the list,
	...
	public List<E> simulateOn(List<E> list) {
		...
		accept(new ListDiffVisitor<E>() {
			...
			public void handleRemove(int index, E element) {
				List<E> first = result.value.subList(0, index);
				List<E> last = result.value.subList(index + 1,
						result.value.size());
	...
	private static class ConcatList<E> extends AbstractList<E> {
		...
		private ConcatList(List<E> firstSublist, List<E> middleSublist,
				List<E> lastSublist) {
		...
		public int size() {
			return firstSublist.size() + middleSublist.size()
					+ lastSublist.size();
		}
	}
